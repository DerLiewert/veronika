...
*/
    ...
    const gsapInit = () => {
        if (isGsapInit) return;
        isGsapInit = true;
        const getPositionFunctions = el => ({
            setRight: (px = 0, elWidth) => {
                const width = elWidth ? elWidth : el.clientWidth;
                return window.innerWidth / 2 - width / 2 - px;
            },
            setLeft: (px = 0, elWidth) => {
                const width = elWidth ? elWidth : el.clientWidth;
                return -(window.innerWidth / 2 - width / 2 - px);
            },
            setTop: (px = 0, elHeight) => {
                const height = elHeight ? elHeight : el.clientHeight;
                return -(window.innerHeight / 2 - height / 2 - px);
            },
            setBottom: (px = 0, elHeight) => {
                const height = elHeight ? elHeight : el.clientHeight;
                return window.innerHeight / 2 - height / 2 - px;
            }
        });
        ...
        const fullText = document.querySelector(".full__text");
        const pieceImgs = document.querySelectorAll(".piece__image");
        const pieceContent = document.querySelector(".piece__content");
        const fullHeight = 500;
        const fullWidth = 500;
        let counter = new Set;
        pieceImgs.forEach((pieceImg, i) => {
            const {setBottom, setLeft, setRight, setTop} = getPositionFunctions(pieceImg);
            if (pieceImg.dataset.beforeX && pieceImg.dataset.beforeY) {
                const posX = pieceImg.dataset.beforeX ? pieceImg.dataset.beforeX.split(",") : null;
                const posY = pieceImg.dataset.beforeY ? pieceImg.dataset.beforeY.split(",") : null;
                gsapWithCSS.set(pieceImg, {
                    x: posX && (posX[0] === "left" ? setLeft(Number(posX[1])) : setRight(Number(posX[1]))),
                    y: posY && (posY[0] === "top" ? setTop(Number(posY[1])) : setBottom(Number(posY[1])))
                });
            } else pieceImg.style.display = "none";
        });
        const onFirstClick = () => {
            if (document.querySelector("body").classList.contains("_is-preloader")) return;
            const intro = document.querySelector(".intro");
            const introImg = intro.querySelector(".intro__img");
            const introSubtext = intro.querySelector(".intro__subtext");
            gsapWithCSS.to(intro, {
                duration: .8,
                zIndex: 0,
                y: -250,
                scale: .4
            });
            gsapWithCSS.to(introImg, {
                opacity: 0,
                duration: .8
            });
            gsapWithCSS.to(introSubtext, {
                opacity: 0,
                duration: .2
            });
            setTimeout(() => {
                document.documentElement.classList.add("_colorfull");
                pieceImgs.forEach((pieceImg, i) => {
                    const {setBottom, setLeft, setRight, setTop} = getPositionFunctions(pieceImg);
                    const options = {
                        duration: 1
                    };
                    const posX = pieceImg.dataset.afterX ? pieceImg.dataset.afterX.split(",") : null;
                    const posY = pieceImg.dataset.afterY ? pieceImg.dataset.afterY.split(",") : null;
                    if (posX) options.x = posX[0] === "left" ? setLeft(Number(posX[1])) : setRight(Number(posX[1]));
                    if (posY) options.y = posY[0] === "top" ? setTop(Number(posY[1])) : setBottom(Number(posY[1]));
                    gsapWithCSS.to(pieceImg, options);
                });
            }, 100);
            setTimeout(() => {
                document.documentElement.classList.add("_img-click");
            }, 2500);
            document.removeEventListener("click", onFirstClick);
        };
        document.addEventListener("click", onFirstClick);
        document.addEventListener("click", e => {
            if (full.classList.contains("_open")) {
                full.classList.remove("_open");
                gsapWithCSS.to(full, {
                    duration: .3,
                    opacity: 0,
                    zIndex: -10
                });
                gsapWithCSS.to(fullText, {
                    duration: .3,
                    opacity: 0
                });
            }
            if (e.target.classList.contains("piece__image") || e.target.closest(".full")) {
                const pieceImg = e.target.classList.contains("piece__image") ? e.target : document.querySelector(".piece__image._full");
                if (!pieceImg) return;
                if (!document.documentElement.classList.contains("_img-click")) return;
                const {setLeft, setTop} = getPositionFunctions(pieceImg);
                const text = pieceImg.dataset.text;
                if (!counter.size) pieceContent.classList.add("_hidden");
                if (!counter.has(pieceImg)) counter.add(pieceImg);
                if (pieceImg.classList.contains("_full")) {
                    if (pieceImg.tagName === "VIDEO") {
                        pieceImg.pause();
                        pieceImg.currentTime = 0;
                    }
                    pieceImg.classList.remove("_full");
                    full.classList.remove("_open");
                    fullImg.style.removeProperty("width");
                    fullImg.style.removeProperty("height");
                    pieceImg.classList.add("_heart");
                    fullText.innerHTML = "";
                    gsapWithCSS.to(pieceImg, {
                        duration: 1,
                        x: pieceImg.dataset.heartX,
                        y: pieceImg.dataset.heartY,
                        width: `${pieceImg.dataset.width}px`,
                        height: `${pieceImg.dataset.height}px`,
                        rotate: pieceImg.dataset.rotate,
                        zIndex: 0
                    });
                    gsapWithCSS.to(full, {
                        duration: .3,
                    ...
                    if (counter.size === pieceImgs.length) gsapWithCSS.to(document.querySelector(".piece__text-end"), {
                        scale: 1,
                        opacity: 1,
                        visibility: "visible",
                        duration: 1,
                    ...
                    const wh = pieceImg.tagName === "IMG" ? pieceImg.naturalWidth / pieceImg.naturalHeight : pieceImg.tagName === "VIDEO" ? pieceImg.videoWidth / pieceImg.videoHeight : 1;
                    let width, height;
                    if (wh <= 1 || wh > 1 && window.innerWidth / 2 - 100 >= fullHeight * wh) {
                        width = fullHeight * wh;
                        height = fullHeight;
                    } else {
                        width = fullWidth;
                    ...
                    fullImg.style.setProperty("width", `${width}px`);
                    fullImg.style.setProperty("height", `${height}px`);
                    fullText.innerHTML = text;
                    pieceImg.classList.remove("_heart");
                    pieceImg.classList.add("_full");
                    setTimeout(() => {
                        full.classList.add("_open");
                    }, 100);
                    gsapWithCSS.to(full, {
                        duration: .3,
                        opacity: 1,
                    ...
                    gsapWithCSS.to(pieceImg, {
                        duration: 1,
                        x: setLeft(fullImg.getBoundingClientRect().right - width, width),
                        y: setTop(fullImg.getBoundingClientRect().top, height),
                        width: `${width}px`,
                        height: `${height}px`,
                        rotate: 0,
                        zIndex: 101
        ...
        const onWindowResize = () => {
            if (!document.documentElement.classList.contains("_colorfull")) pieceImgs.forEach((pieceImg, i) => {
                const {setBottom, setLeft, setRight, setTop} = getPositionFunctions(pieceImg);
                if (pieceImg.dataset.beforeX && pieceImg.dataset.beforeY) {
                    const posX = pieceImg.dataset.beforeX ? pieceImg.dataset.beforeX.split(",") : null;
                    const posY = pieceImg.dataset.beforeY ? pieceImg.dataset.beforeY.split(",") : null;
            ...
            }); else pieceImgs.forEach((pieceImg, i) => {
                const {setBottom, setLeft, setRight, setTop} = getPositionFunctions(pieceImg);
                if (pieceImg.classList.contains("_heart")) gsapWithCSS.set(pieceImg, {
                    x: pieceImg.dataset.heartX,
                    y: pieceImg.dataset.heartY
                }); else if (pieceImg.classList.contains("_full")) {
                    const wh = pieceImg.tagName === "IMG" ? pieceImg.naturalWidth / pieceImg.naturalHeight : pieceImg.tagName === "VIDEO" ? pieceImg.videoWidth / pieceImg.videoHeight : 1;
                    let width, height;
                    if (wh <= 1 || wh > 1 && window.innerWidth / 2 - 100 >= fullHeight * wh) {
                        width = fullHeight * wh;
                    ...
                    if (posX) options.x = posX[0] === "left" ? setLeft(Number(posX[1])) : setRight(Number(posX[1]));
                    if (posY) options.y = posY[0] === "top" ? setTop(Number(posY[1])) : setBottom(Number(posY[1]));
                    gsapWithCSS.set(pieceImg, options);
                }
            });
        };
        onWindowResize();
    ...
    function setSizeInfo() {
        document.querySelector(".window-size-width").innerHTML = "";
        document.querySelector(".window-size-width").innerHTML = getWindowWidth();
        document.querySelector(".window-size-height").innerHTML = "";
        document.querySelector(".window-size-height").innerHTML = getWindowHeight();
    }
    function watchResize() {
        if (getWindowWidth() < 1500 || getWindowWidth() > 1800 || getWindowHeight() < 700 || getWindowHeight() > 1e3) {
    ...
    document.querySelector("button").addEventListener("click", () => {
        if (getWindowWidth() < 1500 || getWindowWidth() > 1800) alert("Ширинаа меньше или больше нужных значений -_-"); else if (getWindowHeight() < 700 || getWindowHeight() > 1e3) alert("Высота меньше или больше нужных значений -_-"); else {
            setTimeout(() => {
